...
- Dentro del item anterior, hay un elemento ``` <table> ``` remplazar el interior del ``` <tbody> ``` con 
	...
	<?php endif ?>
	```
- Al final del mismo archivo index agregar el siguiente código
	```
	<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
...
1.- checar la pendejadisisima del carrito que no le entendi ni vergas

2.- meter regex que solo reemplace los &lt; cuando esten dentro de un string
...
4.- quitarle la mierda esa a la funcion esta de js del archivo bundle

5.- testear si la pagina funciona xd
		...
		<td style="width: 24.1899%;">
			<div style="text-align: center;">
				<?php if($order->order_status->id == 2): ?>
					...
					<br>
				<?php endif ?>
				<a href="<?= BASE_PATH ?>order/<?= $order->folio ?>/">
					Detalles
				</a>
			</div>
		</td>
...
- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_city  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="ciudad"  ``` y asignar el atributo value a ``` value="<?= $order->address?->city ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_postal_code  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="codigo_postal"  ``` y asignar el atributo value a ``` value="<?= $order->address?->postal_code ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_first_name  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="nombre"  ``` y asignar el atributo value a ``` value="<?= $order->address?->first_name ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_phone_number  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="numero_celular"  ``` y asignar el atributo value a ``` value="<?= $order->address?->phone_number ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_state  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="estado"  ``` y asignar el atributo value a ``` value="<?= $order->address?->state ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_street_and_use_number  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="calle"  ``` y asignar el atributo value a ``` value="<?= $order->address?->street_and_use_number ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_last_name  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="apellido"  ``` y asignar el atributo value a ``` value="<?= $order->address?->last_name ?>" ```

- Dentro del mismo archivo index ubicar el item que contiene la clase ``` address_email  ```, dentro de ese elemento ubicar el input y asignar el atributo name a  ``` name="correo"  ``` y asignar el atributo value a ``` value="<?= $order->client->email ?>" ```
...
### Paso 17:
- En la carpeta order/index.php colocar el siguiente código en la cabecera 
...
- Buscar el span con clase ``` order_subtotal  ``` y reemplazar el contenido con 
```
$<?= number_format(($order->total),2) ?>
```

- Buscar el span con clase ``` order_shipping_cost  ``` y reemplazar el contenido con 
```
$<?= number_format(($order->shipping_cost ?? 0),2) ?>
```

- Buscar el span con clase ``` order_total  ``` y reemplazar el contenido con 
```
$<?= number_format(($order->total),2) ?>
```

- Buscar el elemento con clase ``` button_payment  ``` y reemplazar con 
```
<?php if($order->order_status->id == 2): ?>
<a href="<?= BASE_PATH ?>tienda/payment/<?= $order->folio ?>/" 
